...
async function initializeBrowser() {
  ...
  await context.route('**/*', async (route, request) => {
    ...
    if (request.resourceType() === 'document' && 
        request.url().includes('genspark.ai')) {
      ...
      await injectFetchInterceptor(page);
       ...
       await page.goto('https://www.genspark.ai/agents?type=moa_chat');
        console.log('Successfully opened genspark');
        ...
        // 
          ...
          console.log('欢迎使用genspark反代，成功启动！By从前跟你一样');
  ...
  await page.evaluate(() => {
    ...
    window.fetch = async function(...args) {
      ...
      if (typeof resource === 'string' && 
          resource.includes('/api/copilot/ask') && 
    ...
    function processEventData(line) {
      ...
      if (line.startsWith('data: ')) {
        ...
        try {
          ...
          console.log('处理事件数据类型:', jsonData.type);
          ...
          if(jsonData.hasOwnProperty("action")&&jsonData.action!=null){
          ...
          // 检查是否为消息字段增量
          if (jsonData.type === 'message_field_delta' && jsonData.hasOwnProperty('delta')) {
            console.log('检测到增量更新:', jsonData.delta);
            
            // 触发自定义事件
            const event = new CustomEvent('streamDataReceived', { 
              detail: jsonData.delta 
            });
            window.dispatchEvent(event);
          }
        } catch (error) {
          console.error('解析 JSON 时出错:', error, '原始数据:', data);
        }
      }
    }
...
async function sendMessage(res3, message) {
  ...
  await page.evaluate(() => {//关闭历史记录
    const iconElement = document.querySelector('.left-head .icon');
    if (iconElement) {
      const event = new MouseEvent('click', {
        view: window,
        bubbles: true,
    ...
    try {
      const element = await page.locator('#Rectangle\\ 34691250');
      ...
      if(element){
        await page.evaluate(() => {
          const svgPath = document.querySelector('[id="Rectangle 34691250"]');
          if (svgPath) {
            // 创建并分发点击事件
            const event = new MouseEvent('click', {
              view: window,
        ...
        if(Upload){
                      ...
                      await page.evaluate((text) => {
                        ...
                        await page.keyboard.press('Control+V');
                          ...
                          try {
                            textarea = await page.getByPlaceholder('Message').first();
                            await textarea.waitFor({ state: 'visible', timeout: 100 });
